...
用户明确要求保留付费LLM并部署剩余内容。本文件记录实现/配置审核依据，**尚不表示完整M3已部署**。本轮不采用暂停付费、取消预算、提高日预算或旧调用旁路。

执行：[连续性计划](../superpowers/specs/2026-09-20-m3-paid-continuity.md)。现网基线为已发布e6469ba/schema b3，完整候选head仍c7f21a9d680e；本切片无新DDL。
...
- 关联161通过/23专用MySQL skip/1773 warnings/32.17s；244 AST及选定E9/F、diff检查通过。本地全套/准确CI进行中。
- 新增一个实际MySQL门禁，总数23：b3旧配置→完整head→Admin审核→两个模型真实SDK/合成HTTP付费成功，审核前不付款。必须由CI及实际候选分别执行；当前本地skip不是通过。
