...
- cf561合并已完成c3a906a并推release；检查15 Markdown/45本地链接/冲突标记及与838整个应用树一致通过。为服务器后续操作也能读到正确M3/回滚/pin说明，最终文档提交后仅正常ff服务器release源码checkout到该文档head，锁内再核确实只有Markdown差异及4容器身份。已验收运行image仍838，不重建/重启；私有目录留源码同步收据，文档CI与应用CI分开标记。
