...
| 0.3 构建/生产配置 | 离线通过 | 3 项测试；两个 Compose 组合 + allowlist，未构建镜像/部署 |
| 0.4 用量类型迁移 | 代码/离线 DDL 完成，实机阻塞 | `c821b4f7d901`；MySQL 空库/旧数据/model diff 待验证 |
| 0.5 LLM 可靠性 | 待实现 | 业务无写入收集阶段→独立用量→一次应用；严格契约/缓存/显式路由仍未修复 |
| 0.6 爬虫基础 | 本地通过 | 13 项离线测试；真实 MySQL 竞争写入、崩溃/消息窗口待 M2 验证 |
| 0.7 邮件模板 | 本地通过 | 两类模板/Top Insights/预览/SMTP 失败记录共 2 项；投递幂等/真实退订仍在后续切片 |

合计 **49 passed**。M0 尚未整体验收，M1–M4 未开始，Agent 未上线。详见 [本批实施记录](../../audits/2026-09-06-m0-implementation.md)。
