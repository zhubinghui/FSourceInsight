...
- 状态：**M0/M0.5/M1及M2-A/B1/B2a已部署；当前应用22c098c/schema b6。离线576通过，准确CI及两真正候选各14项MySQL通过，私有证据Docker卷/公网/worker门禁通过。完整M2审批/可靠运行/自动schema路由及M3–M4仍待实施，旧日常爬虫链不变。** 新证据见[M2当前成果发布](../../audits/2026-09-09-m2-partial-release.md)；下文保留历史各切片验收。用户新授权发布已完成部分，不借旧CI/候选结果证明新代码。
...
**M1 本地完成条件已满足**：无需真实LLM的fixtures通过；相同快照/recipe/profile在预算内可复现；无新增配置执行代码入口。该声明按上述收紧范围，不包含全部旧源迁移、自动发布、持久化版本优化或实机上线验收。用户授权的[M1受控发布](2026-09-07-m1-release.md)已完成，备份与旧四镜像保留。下一阶段优先M2，使新引擎进入版本/审批/可靠调度链路。

## M2：版本/幂等/调度与可靠交付

用户已选择真实后台HTTP作为主要验收入口。[本地实施细化](2026-09-07-m2-versioned-runtime.md)中的A候选保存切片完成：26项HTTP+2项迁移、全套487通过/13 MySQL专用skip；[报告](../../audits/2026-09-08-m2a-candidate-http.md)。随后[B1后台预览](../../audits/2026-09-08-m2b1-admin-preview.md)也本地完成，当时525通过/14 MySQL专用skip；其后[B2a私有证据/回放](../../audits/2026-09-08-m2b2a-private-evidence.md)也完成，最新574/14，原文默认不保存、显式受限保留24h，仍没有持久policy/独立验证/审批/激活；2.1/2.4其余部分和2.2/2.3仍未完成，未提交部署。
