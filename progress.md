...
## 2026-09-24 M3.4c生产发布（用户授权“合入部署”）
- master快进至cd8dfff后按计划发布；本会话禁止生产写入，由用户以`!`运行审阅过的脚本，其余只读。
- 第1次16:59Z：镜像依赖一致、备份与迁移d3e7完成，新web健康，但注册检查因`celery inspect`逐节点流式输出+`grep -q`+pipefail断管恒失败，脚本按设计17:02:43Z恢复ner镜像；无数据库恢复。诊断容器8秒注册成功。先记计划再修脚本。
- 第2次17:13Z：RELEASE_OK m34c-20260924171323，生产7ec578c/d3e7a1c95b28；备份20507470字节已校验；账本前后一致（且与第1次前相同）；四应用restart0/无错误；恢复任务17:16:11Z成功；公网health/公司列表/公司详情/API/日期筛选均200。
- 审计docs/audits/2026-09-24-m34c-release.md；回滚用ner pin。未做真实模型/抓取/邮件验收。

## 2026-09-24 提交、推送与CI（用户授权）
- 分支m3-company-refresh：0c91f4b日期、589246f M3.4c+JSON/趋势、c6c521a M3.3c、e15c4ed M3.1c、53487cc文档，已推送；未合入master、未部署。
- CI36020415131：离线成功，MySQL 22/23；失败为b5旧schema上用当前ORM插预留（新列company_refresh_id不存在），测试造数问题。b649841改原生SQL插入，断言不变。
- CI36023303297全部成功：离线1141 passed/23 skipped（898秒）；实际MySQL 23/23（46秒），迁移c7→d3e7实跑；真实Admin/Redis/prefork回收100任务、RSS451248KiB回收后下一任务正常。

## 2026-09-24 M3.1c学习输入字节上界（本地切片完成，未提交/部署）
- 用户对“学习专用小输入上界+应用按字节保证”回复“OK，继续”。字节级BPE下token≤UTF-8字节；上界=内容+角色字节+8/条+64。仅限运维声明的供应商（新配置默认openai）。认领按最小审核输入上界裁剪样本，放不下则blocked不付费；准入再复核。
- 3个HTTP回归先red（旧代码发送3271字节上界>2500、300上限仍付费、未声明供应商仍付费）。夹具先后试15000（超$0.20致假通过）、11000（破坏9个金额断言），最终7168/1024/max_tokens1024保持0.092160预留，未改断言。
- 全套1141 passed/23 skipped/666秒。生产启用仍需新建审核配置、开开关与专用worker验证。

## 2026-09-24 M3.3c通用留出（本地切片完成，未提交）
- 用户“同意。继续完成M3开发”；已按同意写入venv位置记忆。
- holdout-validation.v2：base去详情模板回放确定清单，文章URL为详情页、其余为清单页；清单页只查字节/正文指纹，详情页另查URL；候选每个列表/feed须产出文章、声明分页须从第2页产出；feed正文不认证。v1记录保留原规则。
- 5个场景旧代码先red（含分页页URL与RSS源URL被误判“已见过”）；v1兼容测试抓到权威校验只认当前版本、升级后全部stale的缺陷并修复。全套1138 passed/23 skipped/654秒。

## 2026-09-24 继续：JSON原地修改丢失与日期过滤（本地小修，未提交）
- 修订历史：`_save_revision`原地append后赋等值列表，SQLAlchemy不发UPDATE；已有<10条修订的公司后续修订全部丢失（分析内容本身保存）。真实HTTP回归先red，复制后追加修复。
- 重复公司合并别名同一缺陷：目标已有别名时被合并公司名称/别名丢失，削弱后续发现/NER去重。回归先red后修复；其他JSON列扫描无同类写法。已丢失的历史无法恢复。
- 日期过滤：新闻页与/api/v1/news的date_to使用当天00:00包含式上界，漏掉当天其余文章。两条HTTP回归先red，改次日00:00排他上界；时区语义未改。改动文件独立，便于单独提交。
- 全量（含JSON修复、不含日期）1128 passed/23 skipped/623.70秒；含日期修复的最终全量见下一条。
- 公司周情绪趋势：同周不同情绪按各自最早日期拆柱、YEARWEEK周日起算、MM/DD字符串跨年错序。两条HTTP回归先red；改为DATE()聚合+Python周一归桶+真实日期排序，去掉页面MySQL专有函数。日期/趋势见docs/audits/2026-09-24-date-and-trend-fixes.md。
- 含日期修复全量1130 passed；含全部改动最终全量1132 passed/23 skipped/0 failed/623.49秒（~/.cache/m34c-full-05.log）。

## 2026-09-23 M3.4c公司refresh迁持久job（本地切片完成，未提交/部署）
- 用户“继续”。master快进至c5d0b0a（生产a93beb8/c7，由并行会话发布）；新分支m3-company-refresh，先写计划再TDD。
- 手动AI Refresh与文章后续refresh改为company_refresh_job：每公司至多一个活跃job，认领时固定analysis_generation，应用前变化判stale不写；预留关联company_refresh_id，最多3个不同配置、未知费用后不再付、无Celery自动付费重试；24h/180s/120s/60s恢复≤50。旧整数消息只记录不付费。模型给出的非法/内网官网不存储。
- 新迁移d3e7a1c95b28（一表一列，expand-only）。10新行为测试+2迁移测试；关联314通过；全量1125 passed/23 skipped（重建venv后）。首轮204失败因/tmp venv午夜被清理，已记录不计。
- 审计docs/audits/2026-09-23-m34c-company-refresh.md，运维docs/ops/company-refresh.md，CLAUDE.md同步。未实跑MySQL/broker/进程门禁。
