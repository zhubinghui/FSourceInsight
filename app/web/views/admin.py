...
def llm_config_delete(config_id):
    ...
    from app.models.llm import LLMReservation
    if (LLMUsageLog.query.filter_by(config_id=config.id).first()
            or LLMReservation.query.filter_by(config_id=config.id).first()):
        # Reservations already own the config before supplier usage is known.
        flash(f'LLM config {name} has usage history or reservations and cannot be deleted. Disable it instead.', 'error')
        return redirect(url_for('admin.llm_config'))
    try:
        ...
        db.session.rollback()
        flash('LLM config has linked records and cannot be deleted. Disable it instead.', 'error')
    else:
...
def _authored_records(user_id):
    from app.models import crawl_schema, crawl_learning
    from app.models.llm import LLMReconciliation
    references = [(model, field) for module in (crawl_schema, crawl_learning)
                  for model in vars(module).values() if isinstance(model, type)
                  for field in ('created_by_id', 'requested_by_id') if hasattr(model, field)]
    references.append((LLMReconciliation, 'actor_id'))
    return any(db.session.query(model.id).filter(getattr(model, field) == user_id).first()
...
def user_delete(user_id):
    ...
    if user.id == current_user.id:
        ...
        flash(f'User "{user.email}" authored crawl or accounting records and cannot be deleted. '
        ...
        try:
            ...
            db.session.rollback()
            flash('User has linked records and cannot be deleted. Deactivate the account instead.', 'error')
        else:
