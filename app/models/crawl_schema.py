...
class CrawlCaptureManifest(db.Model):
    __tablename__ = 'crawl_capture_manifest'

    id = db.Column(db.Integer, primary_key=True)
    profile_id = db.Column(db.Integer, db.ForeignKey('crawl_source_profile.id'), nullable=False)
    version_id = db.Column(db.Integer, db.ForeignKey('crawl_schema_version.id'), nullable=False)
    # Not an FK: bounded preview reports may be pruned without deleting history.
    preview_report_id = db.Column(db.Integer, nullable=False, unique=True)
    ...
    __table_args__ = (db.Index('idx_capture_profile_id', 'profile_id', 'id'),
                      db.UniqueConstraint('profile_id', 'sequence', name='uq_capture_profile_sequence'))
