...
Executed by the `app.llm.article_tasks.process` consumer of a durable `article_llm_job` (rate-limited 10/min; on branch `m2-activation-routing`, not yet deployed — production still uses the direct `process_article_llm` task):
...
M2 closure (**implemented on branch `m2-activation-routing`, not deployed**, migration `b9d4f6a2c813`, expand-only): Admin approve/reject/rollback/retire of crawl recipes with evidence (current `passed` holdout for learned candidates; a `ready`, current, <24h preview under the saved policy for hand-written ones), CAS on `crawl_source_profile.activation_generation` and an append-only `crawl_schema_decision` audit. Every source has a `crawl_source_state` row: one next-due rule (frequency plus daily anchor, 30-minute gap, retry backoff, 24h cooldown and attention for access failures), a 15-minute lease with a monotonic fence, and one final transaction that writes articles, `article_llm_job` outbox rows, the run log and the schedule. Sources routed to an approved recipe never fall back to their legacy crawler. `BaseCrawler.run(claim)` and `CrawlEngine.run(claim)` require a claim (tests use `tests/support/runs.claimed`). See `docs/superpowers/specs/2026-09-25-m2-activation-routing-design.md`, `docs/superpowers/plans/2026-09-25-m2-activation-routing.md` and `docs/ops/crawl-runtime.md`.
...
**Recreation must use base→prod→caddy→evidence plus the current pin `/home/ubuntu/fsourceinsight-backups/m34c-20260924171323/candidate.compose.json`.** Earlier Admin pins (including the separate16:25 publication) are historical, not current. Floating Dockerfile rebuilds may change dependencies; preserve the accepted runtime or review them separately. Never automatically revive old unfenced paid callers or restore a whole DB to erase uncertain charges after c7. Learning-worker RO/fault/capacity, broader exposure/holdouts, complete recovery/other refresh paths and M2 publication/routing remain unfinished. See `docs/audits/2026-09-20-m3-paid-release.md`, `docs/audits/2026-09-20-m3-paid-continuity.md` and `docs/ops/llm-budget-accounting.md`.

### Celery Configuration

`celery_app.py` — three queues (crawl, llm, email) with explicit task routing. Task modules must be in the `include` list or they won't be discovered by workers. Deployed M3.4b adds `app.llm.startup_tasks` on `llm` and a 60s recovery schedule; it does not add worker capacity or migrate every company-refresh path. Beat schedule (production, until the M2 closure below is released): crawl check every 600s (the task additionally has a default six-hour Redis gate), hourly daily-crawl offers with an exact DB-configured local-hour check (default 01:00 Paris), digest at 07:00 Paris, health check every 6h. On branch `m2-activation-routing` (not deployed) beat instead runs `app.crawlers.tasks.dispatch_due_crawls` and `app.llm.article_tasks.recover` every 60s; the two old crawl entries are removed and their task names only return `skipped`. See `docs/ops/crawl-runtime.md`.
