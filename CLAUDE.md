...
Executed by the `app.llm.article_tasks.process` consumer of a durable `article_llm_job` (rate-limited 10/min, deployed 2026-09-25; the old `process_article_llm` task only converts messages into jobs):
...
Responses are cached in Redis for 7 days, keyed by complete effective messages, prompt/contract versions, provider/model/endpoint and generation parameters. Only validated responses are cached; malformed JSON and truncated/refused output are paid failures, not fabricated empty results. Usage/cost is committed independently to `llm_usage_log`; never call the client while holding flushed business write locks. M3.1a is **deployed with 83813e2/c7**: DB-backed pre-call reservations and immutable Admin reconciliation (`app/llm/budget.py`, migration `a8d31c5e7902`). Each config needs reviewed complete billable input/output ceilings and prices; the migration leaves legacy caps NULL, but both active production OpenAI configs were explicitly reviewed at cutover. They use the official global endpoint, standard `service_tier=default`, 400000 input/4096 total output ceilings (including reasoning), standard prices, and the unchanged $5 daily budget. Effective tier is included in the response-cache key; other endpoints/providers need separate review. Unknown/in-flight costs retain reservations across days; settlement failure never triggers paid fallback. A supplier violating its declared billing caps can still overcharge: the app records/blocks overruns, not guarantees external invoices. Do not deploy this as a transparent upgrade or mix old/new paid workers. Learning code is installed but disabled; restricted holdouts are implemented, while general coverage and dedicated-worker/fault validation remain pending. See `docs/ops/llm-budget-accounting.md` and `docs/audits/2026-09-20-m3-paid-release.md` for current 1083 offline/23 actual-MySQL/image/release evidence. Early 729/18 figures in the 1a audit are historical.

### Crawler System

M3.1b code is **installed, disabled by default and not enabled in production** (`c4e92f7a610b`): explicit Admin start/status/cancel from a retained preview, current persistent policy/history/evidence checks, DB-owned session/attempt identity and pre-call exposure fingerprints, explicit `crawl_schema` routing, bounded offline training replay and candidate-only results. Limits: 3 rounds, $0.20/session, $1/Agent/day and $1/source/day (UTC), 20,000 cumulative billable tokens; all reservations share the global admission transaction. The 180s deadline is cooperative, not an external hard kill. `queued` persists dispatch intent; enabled beat recovers it on `crawl_learn`, while expired running/unknown work is blocked rather than paid again. Learning does not use the response cache. `awaiting_validation` is NOT independent validation or approval. No Article writes/activation. Later slices below add restricted holdouts, cooldown/safe retry, worker configuration and startup-discovery queuing. Current production evidence remains web-only; general exposure/holdout coverage, true broker/MySQL crash gates and measured worker capacity remain pending. Historical 1b local tests: 768 passed / 19 MySQL skipped; cumulative release gates below do not prove dedicated learning-worker readiness. See `docs/audits/2026-09-19-m31b-learning-sessions.md` and `docs/ops/crawl-learning.md`. Do not enable unfinished learning or treat its hashes/configuration as approval.

M2 closure (**deployed 2026-09-25 at `f0312b2`**, migration `b9d4f6a2c813`, expand-only; no recipe approved yet, so every source still uses its legacy crawler): Admin approve/reject/rollback/retire of crawl recipes with evidence (current `passed` holdout for learned candidates; a `ready`, current, <24h preview under the saved policy for hand-written ones), CAS on `crawl_source_profile.activation_generation` and an append-only `crawl_schema_decision` audit. Every source has a `crawl_source_state` row: one next-due rule (frequency plus daily anchor, 30-minute gap, retry backoff, 24h cooldown and attention for access failures), a 15-minute lease with a monotonic fence, and one final transaction that writes articles, `article_llm_job` outbox rows, the run log and the schedule. Sources routed to an approved recipe never fall back to their legacy crawler. `BaseCrawler.run(claim)` and `CrawlEngine.run(claim)` require a claim (tests use `tests/support/runs.claimed`). Admin **Pause/Resume** (`crawl_source_state.paused_at`, migration `c2e8a4f6b917`, deployed 2026-09-26) stops scheduling without invalidating policy, evidence or approvals; the On/Off toggle still makes them stale. See `docs/superpowers/specs/2026-09-25-m2-activation-routing-design.md`, `docs/superpowers/plans/2026-09-25-m2-activation-routing.md` and `docs/ops/crawl-runtime.md`.
...
M1.2 adds `SafeFetcher(FetchPolicy(...))` in `app/crawlers/fetcher.py`. Reuse it within a `with` block for a single synchronous run: approved hosts, pinned public IPs with original-host TLS, robots, shared byte/request/deadline limits. RSS/HTML base fetch methods and `fetch_website_excerpt()` now use it; source classes with their own requests/cloudscraper paths are **not yet migrated**. Startup discovery reuses this boundary in the deployed M3.4b below; its legacy directory parser is not the M1 quality engine or a supervised parsing sandbox. Do not add direct HTTP fallbacks after a safety refusal. `document_url` is private resolution data (may contain query secrets); never log/export the whole response. HTTP tests must use the external `fetch_network` fixture, not mock our fetcher or launch an uninstrumented HTTP helper. The original local-only M1.2 audit is historical; see `docs/audits/2026-09-06-m12-safe-fetch.md` for boundary limits and the cumulative release below for deployment evidence.
...
M3.2c code is **installed behind disabled learning, safe zero-admission retry, NOT paid-work takeover** (`f8b64d2c901e`): failed/cancelled learning sets a >=6h source cooldown; new captures cannot bypass it. Blocked sessions may receive an audited Admin retry only before their original deadline/round limit and only if the entire session has NEVER had a provider reservation, including settled/reconciled/zero-cost ones. Original identity, exposure, money, limits, cooldown and deadlines persist. Retry counter/sequence/input-bound hashes participate in bounded history checks. Tasks preallocate attempt identities before claim COMMIT; error handling fences acquired attempts so stale or unclaimed messages cannot stop a newer running owner. Post-model checks precede parser startup; recovery closes calling attempts without releasing holds or retrying paid uncertainty. Migration adds two session columns and one audit table, conservatively quarantining old failures from migration time without inventing completion timestamps. Historical 2c offline suite: 878 passed / 19 real-MySQL skips, 205 ASTs / 48 templates. Real service/fault/capacity gates and the remaining M3 items above are still pending. See `docs/audits/2026-09-19-m32c-learning-lifecycle.md`.

M3.4a is **shipped configuration/CLI, worker NOT enabled or capacity-validated**: optional `docker-compose.learning.yml` follows prod/(caddy)/evidence. `crawl-learning` profile plus the separate default-off learning flag are required; the layer gives only `worker_learn` an RO/nocopy view of web's external private evidence volume. Existing workers remain unchanged. New worker: prefork1/prefetch1, 50-task/393216KiB post-task recycling, soft180/hard195, provisional 1GiB/1CPU/PIDs64, RO root/64MiB tmpfs/cap-drop/init. `scripts/learning_worker.py run` checks enabled/0700 UID-owned RO evidence/current schema before executing only its reviewed command; `check` queries this learn node's registered tasks, actual queue/routing/exchange/pool/prefetch/recycling/timeouts, without business dispatch. Snapshot success is explicitly not live readiness. This does not verify provider pricing, source authority, actual cgroups/capacity, all other workers, beat health or exact-once recovery. Historical 4a offline suite: 924 passed / 19 actual-MySQL skips; 209 ASTs/48 templates, real Compose offline merge. No dedicated learning-worker start, actual RO mount or prefork/timeout/fault/capacity proof. The 4a milestone had schema f8b64d2c901e; 4b/2d below advance it. Full recovery, service gates and general holdout coverage remain pending. See `docs/audits/2026-09-19-m34a-learning-worker.md` and `docs/ops/learning-worker.md`.

M3.4b is **deployed initial enterprise analysis queuing, NOT complete refresh/recovery coverage** (`a2f6d9b3107c`): new Company + source-owned `StartupAnalysisJob` commit atomically; existing companies/aliases are not reassigned or automatically reanalyzed and failure counters are not reset. `app.llm.startup_tasks` consumes job IDs on `llm`; the crawl producer never invokes the model. Frozen inputs, ORM source/company generations, claim ownership, prompt/configuration checks and global reservation associations fence duplicates, ABA, manual edits, unknown costs and late results. Cache hits remain free but need a current job/input; at most three suppliers, no automatic paid takeover. Queued intent lasts 24h, execution at most180s logically; 60s recovery scans at most50 due records with >=120s durable redispatch spacing. Final deadline checks follow locking reads, but are not SQL/SDK hard deadlines. Directory SafeFetcher is limited to its configured host, 30s/6 requests/512KiB page/2MiB total; scans cap at50 sources/20 new companies per source/1000 extracted entries/4096 alias rows. No homepage fetch, private learning-volume access, Article writes or recipe activation. Source flags/ordinary company routing apply independently of `CRAWL_LEARNING_ENABLED`. Existing manual/article refresh and complete exposure coverage are not migrated. Admin lists bounded historical status or an exact job via accounting links; metadata/fees persist beyond queue expiry. Migration preserves old data/unknown charges and creates no legacy jobs; mixed old writers/bulk SQL/rollback do not enforce the new generation model. Historical 4b full local suite: **988 passed / 20 actual-MySQL skips**, 64 new offline tests, 218 ASTs/48 templates. Cumulative MySQL/base-worker/image/deployment gates are recorded below; dedicated fault/capacity and complete refresh recovery remain unverified. See `docs/ops/startup-analysis.md` and `docs/audits/2026-09-19-m34b-startup-analysis.md`.

M3.2d code is **installed behind disabled learning, NOT paid takeover or complete broker-failure validation** (`b5d81e6a430f`, parent `a2f6d9b3107c`): `learning-delivery.v1` messages carry session ID and a key binding input hash, consumed rounds and retry count. Replayed old messages cannot claim a valid newer phase; unclaimed old errors cannot close it. Known corrupt history is still quarantined. Only an acknowledged rejected-round commit returns a continuation key; lost ACK leaves the current queued phase for recovery. A nullable `dispatch_due_at` and index reserve publication slots before broker I/O, at least30 seconds from recorded UTC admission time; slow COMMIT/RPC can outlive the interval, so this is not physical send-rate control or a publisher lease. Recovery selects at most50 due queued/expired or unconfigured active rows, never repays running uncertainty. Legacy NULL rows get no fabricated delivery permission and old single-argument messages are ignored; terminal candidate/history reads remain compatible. Final claim/admission deadline checks follow expensive reads; no SQL/SDK hard deadline or deadline extension. Admin displays eligibility, not a broker receipt or authorization. Historical 2d full local suite: **1019 passed / 20 actual-MySQL skips**, 31 new offline tests, 223 ASTs/48 templates. Cumulative ordinary-service gates below do not prove learning-worker capacity or all delivery failures; M3 remains incomplete. See `docs/audits/2026-09-19-m32d-learning-dispatch.md` and `docs/ops/crawl-learning.md`. Coordinate stopping all old paid callers before any separately authorized migration/rollback; old code does not enforce this fence.

M3.3c is **installed at `7ec578c` behind disabled learning** (no DDL): new holdout freezes use `holdout-validation.v2`. The base recipe without detail templates is replayed on the pinned capture; its article URLs are detail pages and all other retained documents are inventory pages (lists, pagination, feeds). Inventory pages need unseen raw bytes and normalized text; details also need unseen distinct URLs. Every candidate list/feed must contribute articles and declared pagination must contribute from page 2+, else `insufficient_list_coverage`; feed-supplied bodies are `feed_content_not_independently_sampled`. Results add `inventory` snapshot ids. v1 records keep single-list rules. Still bounded by 6 retained documents. See `docs/audits/2026-09-24-m33c-general-holdouts.md`.

M3.4c is **deployed at `7ec578c` / `d3e7a1c95b28` (2026-09-24 17:15Z)** (parent `c7f21a9d680e`): manual *AI Refresh* and article-triggered refreshes of already-analysed companies now go through `company_refresh_job` (`app/llm/company_refresh.py`, consumer `app.llm.refresh_tasks`). At most one queued/running job per company (unique `active_company_id`); extra requests are absorbed. `process_article_llm` no longer calls the model for company analysis. Claim pins `Company.analysis_generation`; a change before the result is applied closes the job `stale` without writing, so manual edits are never overwritten. Reservations link via `llm_reservation.company_refresh_id`; at most 3 distinct-config attempts, none after unknown cost, no automatic Celery paid retry. 24h queue, 180s cooperative execution, 120s redispatch spacing, 60s recovery of at most 50. Model-supplied websites are stored only if they pass the discovery website check. The retired integer-argument `refresh_company_analysis` message logs and never pays; drain the llm queue before switching and do not mix old/new paid workers. Admin company page shows the latest job state. No refresh-frequency policy, real MySQL/broker/process-kill gates or deployment. See `docs/ops/company-refresh.md` and `docs/audits/2026-09-23-m34c-company-refresh.md`.

Ecosystem map quality (**deployed at `b08fb8d`, admin safety fixes at `e6469ba`; schema `b3d5e8a1c407`**): map scope is the Isère département. `Company.review_status` (pending/approved/rejected) gates the public map; discovery creates `pending` rows, never scans `research_lab` sources, uses free-text extraction only as a fallback, and reads postcode/organisation type from member-directory detail pages through SafeFetcher. Rejected rows are kept on purpose: their slug blocks rediscovery, so reject instead of deleting junk. `entity_type` drives the Schools & Research and collapsed Ecosystem support groups. Admin: review tabs/approve/reject on `/admin/companies`, `/admin/companies/duplicates`, field-preserving merge. Reviewed bulk corrections go through `scripts/apply_ecosystem_review.py` (dry run by default, slug + exact-name guard). The fixture seed no longer re-flags `is_grenoble`. Sector inference is unchanged; a tie-break hypothesis was tested against Minalogic themes and rejected. See `docs/audits/2026-09-19-ecosystem-map-validation.md` and `docs/audits/2026-09-20-ecosystem-map-release.md`.

### Current release integration (deployed; learning remains disabled)

**Production is `9491927` / schema `c2e8a4f6b917` since 2026-09-26 13:32Z** (crawl pause release, pin `~/fsourceinsight-backups/pause-20260926133110/candidate.compose.json`, rollback pin `m2-20260925094001`; see `docs/audits/2026-09-26-pause-release.md`). The M2 closure release (`f0312b2` / `b9d4f6a2c813`, 2026-09-25 09:41Z) is recorded in `docs/audits/2026-09-25-m2-release.md`. The previous M3.4c release (`7ec578c` / `d3e7a1c95b28`, 2026-09-24 17:15Z) is recorded in `docs/audits/2026-09-24-m34c-release.md`. The history below describes the 2026-09-20 base: 83813e2 / schema c7f21a9d680e, released 17:39:41Z with paid AI preserved. M3 base751ac51, ecosystem/Admin changes and audit deletion guards were integrated without dropping parallel work; cf561dc's later ecosystem release documentation is retained. The c7 merge joins b3 and b5 additively. Discovery keeps pending review, rejected tombstones, no lab scanning and same-host facts outside write locks; `startup-analysis.v2` binds review/geography and generations without re-signing old jobs. Explicit official-standard OpenAI terms preserve ordinary paid routes, model assignments and the $5/day budget; old usage remains historical estimates, not certified invoices. Learning stays off: these 400000-input ceilings do not fit its 20000-token allowance. M3.1c (user-approved 2026-09-24, installed at `7ec578c`, learning still disabled) replaces guessing with a provable bound: a separate `crawl_schema`-only config with a small reviewed input ceiling, prompts byte-cropped at claim and re-checked at admission so UTF-8 bytes plus chat overhead never exceed it, only for providers listed in `CRAWL_LEARNING_BYTE_BOUND_PROVIDERS` (default `openai`). See `docs/audits/2026-09-24-m31c-learning-input-bound.md`.

Exact CI35523984792 passed1083 offline/23 actual MySQL and real broker/prefork recycling. Actual web/worker images each passed23 MySQL before AND after deployment,49 templates/Admin, plus ordinary-worker and configuration-CAS/projection CLI gates. Full104-package/Python/OS runtime parity retained LiteLLM1.101.0/OpenAI2.54.0. Fresh backup, empty old queues/TERM-only stop, b3→c7 migration, reviewed config CAS,20-table old projection checks/model diff0/volume and resource protection, beat-last restoration and exact11-container/1-network/2-volume cleanup succeeded. No manual real model/news/SMTP acceptance, no rollback; short clean memory observations are not long-term capacity proof.

**Recreation must use base→prod→caddy→evidence plus the current pin `/home/ubuntu/fsourceinsight-backups/pause-20260926133110/candidate.compose.json`.** Earlier Admin pins (including the separate16:25 publication) are historical, not current. Floating Dockerfile rebuilds may change dependencies; preserve the accepted runtime or review them separately. Never automatically revive old unfenced paid callers or restore a whole DB to erase uncertain charges after c7. Learning-worker RO/fault/capacity, broader exposure/holdouts and complete recovery/other refresh paths remain unfinished; M2 publication/routing is deployed (no recipe approved yet). See `docs/audits/2026-09-20-m3-paid-release.md`, `docs/audits/2026-09-20-m3-paid-continuity.md` and `docs/ops/llm-budget-accounting.md`.

### Celery Configuration

`celery_app.py` — three queues (crawl, llm, email) with explicit task routing. Task modules must be in the `include` list or they won't be discovered by workers. Deployed M3.4b adds `app.llm.startup_tasks` on `llm` and a 60s recovery schedule; it does not add worker capacity or migrate every company-refresh path. Beat schedule (since 2026-09-25): `app.crawlers.tasks.dispatch_due_crawls` and `app.llm.article_tasks.recover` every 60s (the old 600s frequency check and hourly daily-crawl offers are removed; their task names only return `skipped`), digest at 07:00 Paris, health check every 6h, plus the existing 60s startup-analysis and company-refresh recoveries. See `docs/ops/crawl-runtime.md`.
