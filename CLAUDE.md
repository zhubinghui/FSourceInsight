...
M2-B2b.2a (**local only, not committed/deployed**): saved Admin previews now append fingerprint-only `CrawlCaptureManifest` records independently of the 20-report pruning/24h raw retention. New migration `f2a67b904d31` adds `capture_generation` and `capture_history_complete`; existing profiles/old-app inserts stay incomplete, new ORM profiles start tracked. Count/sequence/current-report coverage plus a counter CAS guard persistence; document shape/hash/input bindings are checked separately. This tracks this Admin preview workflow only, not every crawler/CLI/model exposure, and is not independent validation or approval. Local 673 passed / 16 dedicated-MySQL skips; new MySQL case has not been run. Before any release, run the new migration and actual-image MySQL gates with fresh authorization; old web rollback must restrict preview writes so it cannot silently bypass tracking. See `docs/audits/2026-09-10-m2-capture-ledger.md`.
